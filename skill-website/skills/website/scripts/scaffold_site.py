...
:root {{
  ...
  /* System font stacks. Nothing is fetched, so nothing can fail to arrive, and
     the page renders in the reader's own UI face at full weight from the first
     paint. Every stack below covers Latin Extended-A, so Turkish sets correctly. */
...
def render_stylesheet(site: dict) -> str:
    ...
    if theme == "light":
        return STYLESHEET.format(colour_scheme="light", accent=site["accent"], dark_block="")
    if theme == "dark":
