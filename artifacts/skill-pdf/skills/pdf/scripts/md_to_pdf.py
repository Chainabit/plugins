...
def _parse_margin(raw: str | None) -> object:
 if raw is None: return None
 text = raw.strip()
 if text.startswith("{"):
  try: return json.loads(text)
  except json.JSONDecodeError as exc: raise PdfError(ErrorCode.INVALID_INPUT, "margin JSON must be an object with top/right/bottom/left") from exc
 try: return float(text)
 except ValueError as exc: raise PdfError(ErrorCode.INVALID_INPUT, "margin must be a number of points, or a JSON object with top/right/bottom/left") from exc
def main(argv=None):
 p=argparse.ArgumentParser(); p.add_argument("input"); p.add_argument("output"); p.add_argument("--title"); p.add_argument("--lang",default="und"); p.add_argument("--page-size",default="A4"); p.add_argument("--orientation",default="portrait"); p.add_argument("--font"); p.add_argument("--margin"); p.add_argument("--css") ; a=p.parse_args(argv)
 try:
  ...
  margin=_parse_margin(a.margin)
  r=PdfService(SecurityPolicy.for_paths(a.input,a.output)).generate_markdown(Path(a.input),Path(a.output),a.title,a.lang,a.page_size,a.orientation,deterministic=False,quality_profile="professional",font=a.font,margin=margin)
