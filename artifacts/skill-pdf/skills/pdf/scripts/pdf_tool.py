...
def main(argv=None):
 ...
 gen=sub.add_parser("generate-markdown"); gen.add_argument("source"); gen.add_argument("output"); gen.add_argument("--title"); gen.add_argument("--page-size",default="A4"); gen.add_argument("--orientation",default="portrait")
 ...
 try:
  ...
  if a.command=="validate":
   r=verify_pdf(Path(a.pdf),Limits()); return emit({"ok":True,"pages":r.pages,"bytes":r.bytes,"version":r.version})
  s=PdfService(policy(a.source if a.command!="manipulate" else a.sources[0],a.output))
  if a.command=="generate-markdown": r=s.generate_markdown(Path(a.source),Path(a.output),a.title,page_size=a.page_size,orientation=a.orientation,deterministic=False)
  ...
  return emit({"ok":True,"output":str(a.output),"pages":r.pages,"bytes":r.bytes,"warnings":list(r.warnings),"backend":s.last_decision.backend if s.last_decision else None})
 except PdfError as e: return emit({"ok":False,"error":{"code":e.code.value,"message":e.message}},2)
 except Exception: return emit({"ok":False,"error":{"code":"internal_error","message":"unexpected operation failure"}},3)
