...
def capability_registry() -> list[BackendCapabilities]:
    ...
    return [
        ...
        BackendCapabilities("weasyprint", flags["weasyprint"], dependencies["weasyprint"][1], frozenset({"ascii_basic_text", "unicode", "font_embedding", "turkish", "rtl", "images", "tables", "rich_markdown", "pagination", "page_breaks", "headers_footers", "math", "complex_typography", "colors_rgb", "print_quality", "metadata"}) if flags["weasyprint"] else frozenset(), ("generate-markdown", "generate-report"), ("network disabled", "active HTML/SVG/remote assets blocked", "no CJK-capable font is provisioned; a CJK document is refused rather than rendered with missing glyphs", "math is laid out from a TeX subset (fractions, roots, scripts, big operators, matrices, cases, aligned blocks, accents, fonts, symbols); an unsupported command is refused by name and line"), "weasyprint", "isolated print renderer for this skill's Markdown and report inputs"),
