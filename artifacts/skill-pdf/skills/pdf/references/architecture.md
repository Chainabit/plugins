...
`PdfService` is the application controller. `DocumentRequirements` is the information expert for semantic needs; `BackendResolver` owns strategy selection; `SecurityPolicy` owns trust boundaries; image/font/math boundaries normalize or reject assets; renderer adapters own provider APIs; verification owns structural and content-present output checks, including the rejection of image data printed as text, plus exact SHA-256 identity; temporary resources and atomic persistence own lifecycle.

`markdown_html` is the information expert for the Markdown dialect: CommonMark block structure plus GFM tables. Python-Markdown parses the source after an adapter rewrites CommonMark container indentation and paragraph interruptions into the layout Python-Markdown reads the same way; the adapter changes no text. Raw HTML stays text, images cross the image boundary as data URIs, and links keep web and mail targets.
...
CLI responses are versioned typed protocol messages. Rendering mechanics do not decide retry or publication. The platform orchestrator consumes exit status and typed failure class, the authoritative validator proves the exact file, and artifact registration publishes only that proven identity. Human-readable messages are diagnostic context, never machine state.
