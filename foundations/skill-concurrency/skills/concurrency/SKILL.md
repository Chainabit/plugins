---
name: concurrency
description: Concurrency engineering for shared state, synchronization, cancellation, deadlocks, starvation, and deterministic tests.
license: Apache-2.0
metadata:
  version: 1.0.0
  discovery: "Concurrency engineering for shared state, synchronization, cancellation, deadlocks, starvation, and deterministic tests."
  layer: foundations
---

# Concurrency

Own actual scheduler/runtime model, ownership transfer, lock scope, cancellation propagation, race detection, and stress evidence. Activate for tasks that explicitly involve concurrency, races, deadlocks, cancellation; do not activate for generic programming, unrelated technologies, or broad methodology that belongs to a foundation skill.
