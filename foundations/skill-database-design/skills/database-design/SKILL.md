...
description: Database design for invariants, normalization, keys, transactions, indexes, lifecycle, and safe schema evolution.
license: Apache-2.0
metadata:
  version: 1.0.0
  discovery: "Database design for invariants, normalization, keys, transactions, indexes, lifecycle, and safe schema evolution."
  layer: foundations
---

# Database Design

Own domain invariants, ownership of writes, cardinality, isolation needs, index evidence, migration rollout, and rollback feasibility. Activate for tasks that explicitly involve database, schema, normalization, indexes; do not activate for generic programming, unrelated technologies, or broad methodology that belongs to a foundation skill.
